<?php
    $pageTitle = 'Share Cart: &quot;' . html_escape($poster->title) . '&quot;';
...
<?php if ($emailSent): ?>
    <p>We just sent an email to <?php echo html_escape($this->emailTo); ?> with a link to your cart.</p>
    <a href="<?php echo html_escape(url('guest-user/user/me')); ?>">Go back to your dashboard</a>
<?php else: ?>
    <p>Enter an email address below and we'll send them a link to your cart</p>
    <form action="<?php echo html_escape(url(array('action'=>'share', 'id'=>$poster->id), get_option('poster_page_path'))); ?>" method="post" accept-charset="utf-8">
        <div class="poster-field">
            <label for="cart-emailTo">Email</label>
